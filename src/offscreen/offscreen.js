...
function pickMimeType(preferred) {
    const candidates = preferred === 'wav'
...
async function startCapture(msg) {
    try {
        ...
        recorder.onerror = (e) => {
            const message = e.error?.message ?? 'Recorder error';
            post({ type: 'OFFSCREEN_ERROR', message });
            cleanupSession();
        };
        recorder.onstop = () => finalize().catch((err) => post({ type: 'OFFSCREEN_ERROR', message: err instanceof Error ? err.message : String(err) }));
        session = {
            stream,
            recorder,
            audioCtx,
