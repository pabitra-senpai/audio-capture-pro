...
async function handleOffscreenEvent(evt) {
    switch (evt.type) {
...
async function handleStopped(meta) {
    const settings = await getSettings();
    await resetStatus();
    await closeOffscreen();
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    ...
    if ('type' in msg && msg.type.startsWith('OFFSCREEN_')) {
        handleOffscreenEvent(msg).finally(() => sendResponse({ ok: true }));
        return true;
    }
    handleBgMessage(msg).finally(() => sendResponse({ ok: true }));
    return true;
});
